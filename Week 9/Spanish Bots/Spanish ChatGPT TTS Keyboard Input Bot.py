...
# Function to translate text to Spanish using Translatepy
def translate_to_spanish(text):
    translator = Translator()
    translated_text = translator.translate(text, destination_language="Spanish")
    return translated_text.result

# Main function to get user input, generate responses using ChatGPT API, translate to Spanish, and play text-to-speech audio
def main():
    ...
    print("Translating response to Spanish...")
    spanish_response = translate_to_spanish(chatgpt_response)
    print(f"ChatGPT (Spanish): {spanish_response}")

    print("Converting text to speech...")
    chatgpt_audio_file = text_to_speech(spanish_response)
